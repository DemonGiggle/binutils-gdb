/* Simulator for the bonjour processor
...
bonjour_extract_unsigned_integer (unsigned char *addr, int len)
{
...
static void
bonjour_store_unsigned_integer (unsigned char *addr, int len, unsigned long val)
{
  unsigned char * p;
  unsigned char * startaddr = (unsigned char *)addr;
  unsigned char * endaddr = startaddr + len;
...
#define NUM_BONJOUR_REGS 17 /* Including PC */
#define NUM_BONJOUR_SREGS 256 /* The special registers */
#define PC_REGNO     16

/* The ordering of the bonjour_regset structure is matched in the
   gdb/config/bonjour/tm-bonjour.h file in the REGISTER_NAMES macro.  */
/* TODO: This should be moved to sim-main.h:_sim_cpu.  */
struct bonjour_regset
{
  word		  regs[NUM_BONJOUR_REGS + 1]; /* primary registers */
...
set_initial_gprs (void)
{
  ...
  for (i = 0; i < NUM_BONJOUR_REGS; i++)
    cpu.asregs.regs[i] = 0;
  for (i = 0; i < NUM_BONJOUR_SREGS; i++)
  ...
  do 
      ...
      if (inst & (1 << 15))
	{
	  if (inst & (1 << 14))
		...
		case 0x02: /* blt */
		  {
		    BONJOUR_TRACE_INSN ("blt");
		    if (cpu.asregs.cc & CC_LT)
		      pc += INST2OFFSET(inst);
		  }		  break;
		case 0x03: /* bgt */
		  {
		    BONJOUR_TRACE_INSN ("bgt");
		...
		case 0x03: /* ssr */
		  {
		    ...
		    BONJOUR_TRACE_INSN ("ssr");
		    cpu.asregs.sregs[v] = cpu.asregs.regs[a];
		  }
		  break;
		default:
		  BONJOUR_TRACE_INSN ("SIGILL2");
		  sim_engine_halt (sd, scpu, NULL, pc, sim_stopped, SIM_SIGILL);
		  break;
		}
	    }
      ...
      else
	{
	  ...
	  switch (opcode)
		...
		BONJOUR_TRACE_INSN ("swi");
		/* Set the special registers appropriately.  */
		cpu.asregs.sregs[2] = 3; /* BONJOUR_EX_SWI */
		...
		BONJOUR_TRACE_INSN ("umod.l");
		cpu.asregs.regs[a] = (av % bv);
	      }
	      break;
	    case 0x35: /* brk */
	      BONJOUR_TRACE_INSN ("brk");
...
static int
bonjour_reg_store (SIM_CPU *scpu, int rn, unsigned char *memory, int length)
{
  if (rn < NUM_BONJOUR_REGS && rn >= 0)
    {
      if (length == 4)
	{
	  long ival;
	  ...
	  /* misalignment safe */
	  ival = bonjour_extract_unsigned_integer (memory, 4);
	  cpu.asints[rn] = ival;
	}
...
static int
bonjour_reg_fetch (SIM_CPU *scpu, int rn, unsigned char *memory, int length)
{
  if (rn < NUM_BONJOUR_REGS && rn >= 0)
    {
      if (length == 4)
	{
	  ...
	  /* misalignment-safe */
	  bonjour_store_unsigned_integer (memory, 4, ival);
	}
...
static sim_cia
bonjour_pc_get (sim_cpu *cpu)
{
  return cpu->registers[PCIDX];
}

static void
bonjour_pc_set (sim_cpu *cpu, sim_cia pc)
{
  ...
  for (i = 0; i < MAX_NR_PROCESSORS; ++i)
    {
      ...
      CPU_REG_FETCH (cpu) = bonjour_reg_fetch;
      CPU_REG_STORE (cpu) = bonjour_reg_store;
      CPU_PC_FETCH (cpu) = bonjour_pc_get;
      CPU_PC_STORE (cpu) = bonjour_pc_set;
