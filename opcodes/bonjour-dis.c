/* Disassemble bonjour instructions.
...
int
print_insn_bonjour (bfd_vma addr, struct disassemble_info * info)
{
  int length = 2;
  int status;
  stream = info->stream;
  ...
  if ((iword & (1<<15)) == 0)
    {
      ...
      opcode = &bonjour_form1_opc_info[iword >> 8];
      switch (opcode->itype)
	{
  ...
  else if ((iword & (1<<14)) == 0)
    {
      ...
      opcode = &bonjour_form2_opc_info[(iword >> 12) & 3];
      switch (opcode->itype)
	{
	case BONJOUR_F2_A8V:
	  fpr (stream, "%s\t%s, 0x%x",
  ...
  else
    {
      ...
      opcode = &bonjour_form3_opc_info[(iword >> 10) & 15];
      switch (opcode->itype)
	{
	case BONJOUR_F3_PCREL:
	  fpr (stream, "%s\t", opcode->name);
